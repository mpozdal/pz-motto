...
import { updateOrder, updateOrderItem } from "../graphql/mutations";
  ...
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ...
        : orderModelProp;
      let userRecord = record ? await record.user : undefined;
      setUser(userRecord);
      const linkedItems = record?.items?.items ?? [];
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          itemsToUnLink.forEach((original) => {
            if (!canUnlinkItems) {
              throw Error(
                `OrderItem ${original.id} cannot be unlinked from Order because orderItemsId is a required field.`
              );
            }
            promises.push(
              client.graphql({
          ...
          const modelFieldsToSave = {
            orderUserId: modelFields?.user?.id ?? null,
            orderStoreId: modelFields?.store?.id ?? null,
