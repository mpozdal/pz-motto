...
import { createOrderItem, updateFlavorShot } from "../graphql/mutations";
...
export default function OrderItemCreateForm(props) {
  const {
    clearOnSuccess = true,
    onSuccess,
    onError,
  ...
  const resetStateValues = () => {
    ...
    setFlavorShots(initialValues.flavorShots);
    setCurrentFlavorShotsValue(undefined);
    setCurrentFlavorShotsDisplayValue("");
    setPrice(initialValues.price);
    setOrderItemsId(initialValues.orderItemsId);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const modelFieldsToSave = {
            orderItemDrinkId: modelFields?.drink?.id,
            orderItemMilkId: modelFields?.milk?.id,
            orderItemSizeId: modelFields?.size?.id,
            price: modelFields.price,
            orderItemsId: modelFields.orderItemsId,
          };
          const orderItem = (
            await client.graphql({
              query: createOrderItem.replaceAll("__typename", ""),
              variables: {
                input: {
                  ...modelFieldsToSave,
                },
          ...
          const promises = [];
          promises.push(
            ...flavorShots.reduce((promises, original) => {
              promises.push(
                client.graphql({
                  ...
                  variables: {
                    input: {
                      id: original.id,
                      orderItemFlavorShotsId: orderItem.id,
                    },
                  },
                })
              );
      ...
      {...getOverrideProps(overrides, "OrderItemCreateForm")}
      {...rest}
    >
      <ArrayField
        lengthLimit={1}
