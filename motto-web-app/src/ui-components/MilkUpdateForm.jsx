...
import { getDrink, getMilk, listDrinks } from "../graphql/queries";
import { updateMilk } from "../graphql/mutations";
...
export default function MilkUpdateForm(props) {
  const {
    id: idProp,
    milk: milkModelProp,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = milkRecord
      ? { ...initialValues, ...milkRecord, drinkMilksId }
      : initialValues;
    setName(cleanValues.name);
    setDrinkMilksId(cleanValues.drinkMilksId);
    setCurrentDrinkMilksIdValue(undefined);
    setCurrentDrinkMilksIdDisplayValue("");
    setErrors({});
  };
  const [milkRecord, setMilkRecord] = React.useState(milkModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getMilk.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getMilk
        : milkModelProp;
      const drinkMilksIdRecord = record ? record.drinkMilksId : undefined;
      const drinkRecord = drinkMilksIdRecord
        ? (
            await client.graphql({
              query: getDrink.replaceAll("__typename", ""),
              variables: { id: drinkMilksIdRecord },
            })
          )?.data?.getDrink
        : undefined;
      setDrinkMilksId(drinkMilksIdRecord);
      setSelectedDrinkMilksIdRecords([drinkRecord]);
      setMilkRecord(record);
    };
    queryData();
  }, [idProp, milkModelProp]);
  React.useEffect(resetStateValues, [milkRecord, drinkMilksId]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        let modelFields = {
          name,
          drinkMilksId: drinkMilksId ?? null,
        };
      ...
      {...getOverrideProps(overrides, "MilkUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || milkModelProp)}
